--------------------------------------------------------
--  DDL for Procedure STAYS_SCL_095_095_ROT_1DEG
--------------------------------------------------------
set define off;

  CREATE OR REPLACE NONEDITIONABLE PROCEDURE "SYS"."STAYS_SCL_095_095_ROT_1DEG" 
(
...
BEGIN
  ...
  declare
    ...
    is
      select
        distinct id
      from T_TRANS_SCL_095_095_ROT_1DEG
        --where id='abboip'
      ;
  begin
      ...
      BEGIN
        ...
        loop
          ...
          if diameter > DISTANCE then
            ...
            /* Insertar stay en S (set of stays) -> t_stays_scl_095_095_rot_1deg */
            /* S? S?(Medoid(R,i, j*),ti ,t j* ) ; */
            insert into t_stays_scl_095_095_rot_1deg
            (
  ...
  END;
END STAYS_SCL_095_095_ROT_1DEG;
