--------------------------------------------------------
--  DDL for Procedure UNIQ_4P_SC_095_095_ROT1D
--------------------------------------------------------
set define off;

  CREATE OR REPLACE NONEDITIONABLE PROCEDURE "SYS"."UNIQ_4P_SC_095_095_ROT1D" 
(
...
BEGIN
  declare
    ...
    TYPE unique_trace IS RECORD (lat001 T_TRANS_SCL_105_09.latitude001%TYPE, long001 T_TRANS_SCL_105_09.longitude001%TYPE, dt T_TRANS_SCL_105_09.datetime%TYPE); -- Record con latitud y longitud con precisión .001 y timestamp
  ...
  begin
    ...
    DBMS_OUTPUT.PUT_LINE('********** Uniqueness of T_TRANS_SCL_105_09 - 4 Points **********');
    ...
    begin
      ...
      loop
        ...
        -- Se salta al siguiente sujeto si no se han encontrado 4 trazas
        --dbms_output.put_line('unique_traces.count:'||unique_traces.count);
        continue when unique_traces.count < 4;
        
        found_n_times := 0;
        
        -- Buscar cantidad de individuos con las mismas trazas que las seleccionadas previamente (4 trazas)
        -- 4o nivel
        select
          count(distinct id)
        into
        ...
        insert into T_UNIQ_4P_SC_095_095_ROT1D
        (
          id,
