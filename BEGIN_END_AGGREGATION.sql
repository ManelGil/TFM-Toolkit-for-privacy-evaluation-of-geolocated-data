--------------------------------------------------------
--  DDL for Procedure BEGIN_END_AGGREGATION
--------------------------------------------------------
set define off;

  CREATE OR REPLACE NONEDITIONABLE PROCEDURE "SYS"."BEGIN_END_AGGREGATION" 
(
  ...
  begin
    ...
    loop
      ...
      > TIME_DURATION then
        ...
        insert into T_BEGIN_END_AGGREGATION (id, latitude, longitude, datetime)
